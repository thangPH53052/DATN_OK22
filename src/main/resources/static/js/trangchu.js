...
function filterProductsName(input) {
	...
	const list = document.querySelectorAll('.sanPham');

	let count = 0;
	list.forEach(sp => {
		const ten = sp.querySelector('h3');
		if (ten && ten.innerText.toUpperCase().includes(filter)) {
			sp.style.display = '';
			count++;
		} else {
	...
	const emptyMsg = document.getElementById('khongCoSanPham');
